...
void C界面图形控制::f标题人物(bool a开关, bool a动画) {
	if (a开关) {
		if (!界面图形::fi使用(m标题人物)) {
			...
			t图形参数 v参数 = t图形参数::c界面;
			v参数.m纹理 = C游戏::fg图形().fg纹理()[L"东方月亮船.灵梦"];
			v参数.m顶点 = C游戏::fg图形().fg顶点矩形()[L"东方月亮船.灵梦"];
			v参数.m坐标 = t向量2(c框架范围x, 0);
			v参数.m图层 = E图层::e人;
			m标题人物 = v图形工厂.f产生图形<界面图形::C标题人物>(v参数);
		} else {
			m标题人物->f出现(!a动画);
		}
...
void C界面图形控制::f云(bool a开关, bool a动画) {
	if (a开关) {
		...
		v参数.m纹理 = C游戏::fg图形().fg纹理()[L"东方月亮船.云"];
		const S顶点矩形 *vp云矩形 = C游戏::fg图形().fg顶点矩形()[L"东方月亮船.云"];
		int i = 0;
		for (auto &vp云 : ma云) {
			if (!界面图形::fi使用(vp云)) {
				v参数.m顶点 = vp云矩形 + i;
				v参数.m坐标.x = c坐标分布x(v随机工厂);
				v参数.m坐标.y = c坐标分布y(v随机工厂);
				v参数.m速度.x = c速度分布x(v随机工厂);
				const int v图层 = v参数.m坐标.y > 界面图形::C云::c层分界线 ? E图层::e后云 : E图层::e前云;
				v参数.m图层 = v图层;
				vp云 = v图形工厂.f产生图形<界面图形::C云>(v参数);
				i = (i + 1) % 8;
			} else {
				vp云->f出现(!a动画);
			}
