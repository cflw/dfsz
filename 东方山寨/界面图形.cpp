...
void C界面图形控制::f背景(bool a) {
	if (a) {
		auto &v画背景 = C游戏::fg图形().fg画背景();
		auto v纹理 = C游戏::fg图形().fg纹理()[L"东方月亮船.背景"];
		v画背景.f切换背景(v纹理, t向量2::fc相同(512));
	}
}
	...
	if (a) {
		...
		for (auto &vp云 : ma云) {
			if (!界面图形::fi使用(vp云)) {
				...
				const int v图层 = v图形工厂.m参数.m坐标.y > 界面图形::C云::c层分界线 ? C界面图形控制::E图层::e后云 : C界面图形控制::E图层::e前云;
				v图形工厂.m参数.m图层 = v图层;
