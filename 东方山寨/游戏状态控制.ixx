...
public:
	...
	C游戏状态控制(C程序实现 &);
	void f切换计算();	//每帧都会调用
	void f切换游戏状态(E游戏状态 状态, E窗口 窗口 = E窗口::e无);
	bool fi游戏中() const;
	//游戏状态切换函数
	static tf切换 fg切换函数(E游戏状态 前, E游戏状态 后);
	bool f切换_载入中_主菜单();
	bool f切换_载入中_游戏中();
	bool f切换_主菜单_游戏中();
	bool f切换_游戏中_主菜单();
...
void C游戏状态控制::f切换游戏状态(E游戏状态 a状态, E窗口 a窗口) {
	m新状态 = a状态;
	m切换窗口 = a窗口;
	mf切换 = fg切换函数(m状态, a状态);
}
bool C游戏状态控制::fi游戏中() const {
	return m状态 == E游戏状态::e游戏中 || m状态 == E游戏状态::e游戏菜单;
}
//游戏状态切换函数
C游戏状态控制::tf切换 C游戏状态控制::fg切换函数(E游戏状态 a前, E游戏状态 a后) {
...
bool C游戏状态控制::f切换_载入中_主菜单() {
	f切入_主菜单();
	m黑屏渐变 = 1;
	return true;
}
bool C游戏状态控制::f切换_载入中_游戏中() {
	f切入_游戏中();
	m黑屏渐变 = 1;
	return true;
}
bool C游戏状态控制::f切换_主菜单_游戏中() {
	if (f切换_渐变滴答()) {
		f切出_主菜单();
		f切入_游戏中();
		return true;
...
bool C游戏状态控制::f切换_游戏中_游戏菜单() {
	f切入_游戏菜单();
	return true;	//弹出什么窗口在 f游戏中弹出菜单 中确定
}
bool C游戏状态控制::f切换_游戏菜单_游戏中() {
	if ((m切换渐变 += c切换渐变速度) >= 1) {
		f切出_游戏菜单();
		return true;
	}
...
bool C游戏状态控制::f切换_渐变滴答() {
	m切换渐变 += c切换渐变速度;
	m黑屏渐变 += c切换渐变速度;
	return m切换渐变 >= 1;
}
void C游戏状态控制::f切入_主菜单() {
	mp实现->m界面->f切换下个窗口(E窗口::e主菜单);
}
void C游戏状态控制::f切入_游戏中() {
	//m任务_资源.wait();
	mp实现->m游戏->f开始();
}
void C游戏状态控制::f切入_游戏菜单() {
	mp实现->m界面->f切换下个窗口(m切换窗口);
}
void C游戏状态控制::f切出_主菜单() {
	mp实现->m界面->f关闭窗口();
	mp实现->m界面图形->f关闭图形();
}
void C游戏状态控制::f切出_游戏中() {
	mp实现->m游戏->f结束();
}
void C游戏状态控制::f切出_游戏菜单() {
	mp实现->m界面->f关闭窗口();
}
}	//namespace 东方山寨