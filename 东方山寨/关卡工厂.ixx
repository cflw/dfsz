...
class I关卡工厂 : public I工厂<C关卡> {
public:
	I关卡工厂(int);
	void f注册(int);
	int fg标识() const;
	void f初始化关卡(C关卡 &) const;	//给关卡赋值
	int m标识 = 0;
};
...
public:
	using I关卡工厂::I关卡工厂;
	std::unique_ptr<C关卡> f接口_新建u() const override {
		auto v = std::make_unique<t关卡>();
		f初始化关卡(*v);
		return v;
	}
	std::shared_ptr<C关卡> f接口_新建s() const override {
...
void I关卡工厂::f初始化关卡(C关卡 &a关卡) const {
	a关卡.m工厂 = this;
}
}	//namespace 东方山寨