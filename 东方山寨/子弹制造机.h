...
public:
	...
	template<typename t, typename...t参数> std::shared_ptr<t> f产生子弹(const t参数 &...a参数);
	C循环 f循环(int);
	...
	S子弹参数 m参数;	//公开的
private:
	C实现 *m实现;	//在游戏实现中管理子弹制造机实现
};
template<typename t, typename...t参数> std::shared_ptr<t> C子弹制造机::f产生子弹(const t参数 &...a参数) {
	static_assert(std::is_base_of<C子弹, t>::value, "必须继承自C子弹");
	if (f实现_i有空()) {
