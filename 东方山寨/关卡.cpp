...
void C关卡控制::f切换_关卡(C关卡 &a关卡) {
	m切换_p关卡 = &a关卡;
	m切换_p关卡组 = nullptr;
}
void C关卡控制::f切换_关卡组(const t关卡组 &a关卡组) {
	m切换_p关卡组 = &a关卡组;
	m切换_p关卡 = nullptr;
}
bool C关卡控制::f切换_计算() {
...
void C关卡控制::f开始关卡组(const t关卡组 &a关卡组) {
	m关卡数量 = a关卡组.size();
	assert(m关卡数量 > 0);
	mp关卡组 = &a关卡组;
}
void C关卡控制::f开始关卡(C关卡 &a关卡) {
	f结束关卡();
	m经过时间 = 0;
	a关卡.m关卡 = this;
	a关卡.f事件_初始化();
	mp关卡 = &a关卡;
	m随机数种子 = (unsigned int)time(nullptr);
	f对象_使用();
}
void C关卡控制::f结束关卡() {
	if (mp关卡) {
...
void C关卡控制::f动作_结束关卡(float a时间) {
	...
	if (m当前关卡序号 < m关卡数量) {
		C关卡 *const v下一关卡 = mp关卡组->at(m当前关卡序号);
		f切换关卡(*v下一关卡, a时间);
	} else {	//没有下一关卡了,结束游戏
		const auto f结束游戏 = []() {
			C游戏::fg内容().f游戏_通关菜单();
...
// 关卡接口
//==============================================================================
void C关卡::f注册关卡(int a标识) {
	C关卡管理::fg实例().f注册关卡(a标识, *this);
}
void C关卡事件::f事件_执行() {
	f动作_结束();
}
...
C切换关卡事件::C切换关卡事件(C关卡控制 &a关卡控制, C关卡 &a关卡):
	m关卡控制(&a关卡控制),
	m关卡(&a关卡) {
}
void C切换关卡事件::f事件_执行() {
