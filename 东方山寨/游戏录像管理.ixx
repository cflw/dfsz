...
public:
	...
	std::shared_ptr<C录像> f打开录像(const std::filesystem::path& a文件名, bool a完整 = true, bool a缓存 = true);
	E录像读写结果 f完整打开录像(std::shared_ptr<C录像>& ap录像);
...
module : private;
namespace 东方山寨 {
C录像机& C游戏录像管理::f新建录像() {	//在进入游戏前调用
	C设置管理& v设置管理 = C设置管理::fg实例();
	m录像.f新建录像(v设置管理.fg程序设置(), v设置管理.fg游戏设置());
	return m录像;
}
void C游戏录像管理::f保存录像() {	//保存当前录制的录像
	m录像.f结束录像();
	const auto v路径 = c录像目录 / f自动录像文件名();
	C录像文件::f保存(v路径, *m录像.m录像);
}
std::wstring C游戏录像管理::f自动录像文件名() {
	return std::format(c紧凑时间格式, 时间::f本地秒(时间::fg本地现在())) + L".zip"s;
}
std::shared_ptr<C录像> C游戏录像管理::f打开录像(const std::filesystem::path& a文件名, bool a完整, bool a缓存) {	//读取录像,返回录像
	if (a缓存) {
...
E录像读写结果 C游戏录像管理::f完整打开录像(std::shared_ptr<C录像>& ap录像) {	//重新载入录像
	return C录像文件::f打开(ap录像->m文件路径, ap录像, true);
}
C回放机& C游戏录像管理::f回放录像(const std::shared_ptr<C录像>& a录像) {
	m回放.m录像 = a录像;
	return m回放;
}
//录像摘要
