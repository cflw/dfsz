...
void C游戏::C实现::f开始(const S游戏设置 &a设置) {
	...
	m难度.m基础难度 = a设置.m基础难度;
	m难度.m增加难度 = a设置.m增加难度;
	//m关卡.f初始化_在游戏开始(a设置.ma关卡, a设置.m关卡数量);	//TODO:游戏设置的关卡不再使用指针,以后修改
	f进入关卡(C关卡管理::fg关卡(a设置.m关卡标识));
}
void C游戏::C实现::f结束() {
	ma子弹.f清空();
	ma玩家子弹.f清空();
...
void C游戏::C实现::f计算() {
	...
	//关卡
	while (m新关卡 != m关卡.m关卡) {	//允许进入关卡时切换关卡,但是要防止死循环
		f实现_进入关卡();
	}
	I游戏输入 &v输入 = fg游戏输入();
...
void C游戏::C实现::f实现_进入关卡() {
	...
	ma子弹.f清空();
	m关卡.f初始化关卡(*m新关卡);
	mp资源->mp游戏输入->f事件_切换关卡(0, &m玩家.m成绩);
	m游戏速度.fs速度(1);
}
void C游戏::C实现::f更新() {
