...
C游戏::C实现::C实现() {
	...
	//子弹工厂
	m子弹工厂.f初始化_环境(m游戏速度);
	m子弹工厂.f初始化_数组(ma子弹, ma子弹图形缓冲);
	m子弹工厂.f初始化_资源(mp资源->ma子弹属性, mp资源->mp图形->fg图形管线数组());
	//敌机工厂
	m敌机工厂.f初始化_环境(m游戏速度);
	m敌机工厂.f初始化_数组(ma敌机, mp资源->mp图形->fg图形缓冲数组());
	m敌机工厂.f初始化_资源(mp资源->ma敌机属性);
	//道具工厂
	m道具工厂.f初始化_环境(m游戏速度);
	m道具工厂.f初始化_数组(ma道具, mp资源->mp图形->fg图形缓冲数组());
	m道具工厂.f初始化_资源(mp资源->ma道具属性);
	//玩家子弹工厂
...
void C游戏::C实现::f开始(const S游戏设置 &a设置) {
	...
	if (a设置.m关卡) {	//单个关卡比关卡组优先
		m关卡.f切换_关卡(v关卡管理.fc关卡(a设置.m关卡));
	} else if (a设置.m关卡组) {
		m关卡.f切换_关卡组(v关卡管理.fg关卡工厂组(a设置.m关卡组));
	} else {
		assert(!"没有关卡");	//没有关卡
	}
}
...
const C敌机制造机 &C游戏::C内容::f工厂_敌机() const {
	return m实现->m敌机工厂;
}
const C玩家子弹制造机 &C游戏::C内容::f工厂_玩家子弹() const {
	return m实现->m玩家子弹工厂;
}
const C道具制造机 &C游戏::C内容::f工厂_道具() const {
	return m实现->m道具工厂;
}
