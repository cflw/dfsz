...
// 画三维管线
//==============================================================================
C画三维管线::C画三维管线(C图形引擎 &a引擎, 三维::C三维 &a三维):
...
void C画三维管线::fs世界矩阵(const 数学::S矩阵4 &a) {
	f刷新();
	m常量v.m世界 = a;
	m更新v = true;
}
void C画三维管线::fs视图矩阵(const 数学::S矩阵4 &a) {
	f刷新();
	m常量v.m视图 = a;
	m更新v = true;
}
void C画三维管线::fs相机(const 数学::S相机 &a) {
	f刷新();
	m常量v.m视图 = a.ft矩阵4r();
	m更新v = true;
	m常量p.m眼睛位置 = a.m坐标;
...
void C画三维管线::fs雾颜色(const 数学::S颜色 &a) {
	f刷新();
	m常量p.m雾颜色 = a;
	m更新p = true;
}
void C画三维管线::fs雾范围(float a开始, float a范围) {
	f刷新();
	m常量p.m雾开始 = a开始;
	m常量p.m雾范围 = a范围;
	m更新p = true;
}
void C画三维管线::fs纹理(const 三维::tp纹理资源视图 &a纹理) {
	if (m纹理 != a纹理) {
		m缓冲.f刷新();
		m纹理 = a纹理;
	}
}
void C画三维管线::f绘制(const VS_3D *ap顶点, size_t a顶点数量, const WORD *ap索引, size_t a索引数量) {
	m缓冲.f复制(ap顶点, a顶点数量 * sizeof(VS_3D), ap索引, a索引数量 * sizeof(WORD));
}
//==============================================================================
// 画三维
//==============================================================================
C画三维::C画三维(C画三维管线 &a图形管线):
	m图形管线(&a图形管线)  {
}
...
void C画三维::f画模型(const C模型 &a) {
	...
	m图形管线->f自动准备();
	m图形管线->fs纹理(m纹理->m纹理);
	m图形管线->f绘制(vp顶点, v顶点数量, a.m索引, a.m索引数);
}
void C画三维::f画平面(int x, int y) {
	...
	m图形管线->f自动准备();
	m图形管线->fs纹理(m纹理->m纹理);
	m图形管线->f绘制(vp顶点, 4, c平面索引, std::size(c平面索引));
}
}	//namespace 东方山寨