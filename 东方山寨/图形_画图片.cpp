...
// 画图片管线
//==============================================================================
C画图片管线::C画图片管线(C图形引擎 &a引擎, 三维::C三维 &a三维):
	m渲染控制{&a三维.fg渲染控制()}, 
	m渲染状态{&a三维.fg渲染状态()}, 
	m缓冲{a三维} {
	三维::C顶点格式 v顶点格式;
	...
	m缓冲.m顶点缓冲.f初始化(sizeof(VS_IMAGE), c顶点缓存数量);
...
void C画图片管线::f绘制(const VS_IMAGE *a顶点, const WORD *a索引) {
	m渲染控制->fs纹理(0, m纹理.Get());
	m缓冲.f复制(a顶点, sizeof(VS_IMAGE) * 4, a索引, sizeof(WORD) * 6);
}
void C画图片管线::fs纹理(const 三维::tp纹理资源视图 &a纹理) {
	if (m纹理 != a纹理) {
		f刷新();
		m纹理 = a纹理;
	}
