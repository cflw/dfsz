...
//循环计算
float C子弹制造机::C循环::C变换::f计算(t标志 a标记) const {
...
t向量2 C子弹制造机::C循环::C变换::f计算_速度变换(float a大小, float a方向, t计算 a计算) const {
	...
	const float v方向 = m速度.fg方向d() + a方向 * v计算;
	return t向量2::fc方向d(v大小, v方向);
}
...
void C子弹制造机::C实现::f初始化_数组(C对象数组<C子弹> &aa子弹) {
	ma子弹 = &aa子弹;
}
...
void C子弹制造机::C实现::f产生子弹(std::shared_ptr<C子弹> a子弹, S子弹参数 &a) {
	...
	if (v子弹->m初始化_样式 < 0) {
		...
		if (a.m缓存_样式.m整数 != v子弹->m初始化_样式) {
			a.m缓存_样式 = ma子弹属性->f找(v子弹->m初始化_样式);
		}
		v子弹->m子弹属性 = a.m缓存_样式.m指针;
	}
	if (v子弹->m初始化_绘制 < 0) {
		...
		if (a.m缓存_绘制.m整数 != v子弹->m初始化_绘制) {
			a.m缓存_绘制 = ma画子弹->f找(v子弹->m初始化_绘制);
		}
		v子弹->m绘制 = a.m缓存_绘制.m指针;
	}
	//数据合法性检查
	assert(v子弹->m缩放.x >= 0);
	assert(v子弹->m缩放.y >= 0);
