...
void C敌机::f计算() {
	...
	f计算运动();
	m动画->f接口_动作(E动画动作::e水平移动, 数学::f取符号<float>(m速度.x, 1));
	m动画->f接口_计算();
}
void C敌机::f更新() {
	m动画->f接口_更新();
...
void C敌机::f显示() const {
	if (fi纹理()) {
		...
		auto &v画图 = C游戏::fg图形().fg画图形();
		v画图.f绘制圆形(t圆形(m坐标, 16));
	}
}
bool C敌机::fi在窗口外() const {
	return C边界::c窗口.f外边判断(m坐标, 32);
}
void C敌机::f中弹(int p伤害) {
	//无敌时不会计算伤害
	if (m无敌时间 <= 0) {
		if (m生命值.m当前 > 0) {
			m生命值.m当前 -= p伤害;
...
const S顶点矩形 &C敌机::fg顶点矩形() const {
	const auto &v顶点 = m敌机属性->m顶点.m指针;
