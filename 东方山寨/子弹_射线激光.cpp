...
void C射线激光::f接口_初始化() {
	...
	m缩放.x = 0;
	m缩放.y = 0;
	m目标.x = (m初始化_长 <= 0) ? (c屏幕范围r / m子弹属性->fg显示x()) : m初始化_长;
...
void C射线激光::f接口_更新() {
	...
	//添加节点
	auto f添加节点0 = [&](float a位置, float p消失) {
		va节点.push_back(t顶点{a位置, p消失});
	};
	f添加节点0(0, 0);
	f添加节点0(v显示长度, 1);
	auto f添加节点1 = [&](float a位置, float p消失) {
		if (a位置 > 0 && a位置 < v显示长度) {
			f添加节点0(a位置, p消失);
		}
	};
	...
	auto f赋消失 = [](t顶点 &p点, float p消失) {
		float &v消失 = p点.m消失;
		v消失 = std::max<float>(v消失, p消失);
	};
...
//初始化
void C射线激光::f初始化_长宽(float p长, float p宽) {
	m初始化_长 = p长;
	m初始化_宽 = p宽;
}
//动作
...
void C射线激光::f消失段_新建(float a位置, float a半径) {
	...
	ma消失段.f新建(1, [&](S消失段 &p, int i) {
		p.v序号 = i;
		p.v下个段 = m开始段;
		p.v范围.fs中心半径(a位置, v半径);
		if (p.v范围.m小 < 0) {
			p.v范围.m小 = 0;
		}
		p.v消失.f初始化(0);
