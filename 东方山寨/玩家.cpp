...
void C玩家::f输入按键(const t向量2 &a方向键, const 输入::C按键组 &a按键) {
	m方向键 = a方向键;
	const bool v低速 = a按键.fg按键((输入::t索引)E按键::e低速).fi按下();
	//处理方向键速度
	if (v低速) {
		m方向键 *= 0.5f;
	}
	m标志[e低速移动] = v低速;
	m标志[e射击] = a按键.fg按键((输入::t索引)E按键::e射击).fi按下();
	m标志[e炸弹] = a按键.fg按键((输入::t索引)E按键::e炸弹).fi刚按下();
}
void C玩家::f初始化_在关卡开始() {
	m自机.f初始化_在关卡开始();
	m子机组.f初始化_在关卡开始();
...
void C玩家::f初始化_在游戏开始(const C游戏设置 &a设置) {
	m成绩.m得分 = 0;
	m成绩.m残机 = a设置.m残机;
	m成绩.m火力 = a设置.m火力;
	m成绩.m炸弹 = a设置.m炸弹;
...
void C玩家::f销毁_在游戏结束() {
	for (auto &vp玩家成绩 : ma玩家成绩) {
		vp玩家成绩->f动作_结束();
	}
	ma玩家成绩.clear();
	m自机.m图形 = nullptr;
	for (auto &v子机 : m子机组.ma子机) {
		v子机.m图形 = nullptr;
	}
}
void C玩家::f初始化_玩家成绩图形() {
	...
	v玩家成绩图形工厂.f产生图形<long long>(f上位置(), v上对齐, t颜色::c灰,
