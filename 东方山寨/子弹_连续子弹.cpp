...
#include "子弹制造机.h"
#include "游戏.h"
#define 连续子弹连接锁 std::lock_guard<std::mutex> v锁(m连接互斥)
namespace 东方山寨 {
//==============================================================================
// 连续子弹的连接工具
//==============================================================================
...
void C连续子弹::f接口_计算() {
	...
	f基础_计算运动();
	f基础_计算方向();
	vi窗口外 = 边框::C矩形::c窗口.f判断(m坐标, m子弹属性->fg显示x(m缩放.x));
	//渐变&消失
	m出现.f计算();
	if (m消失.fi消失()) {
		m消失.f计算();
...
void C连续子弹::f接口_更新() {
	...
	if (m后) {	//连续子弹
		...
		while (true) {
			C子弹顶点::C曲线::t顶点 v原始顶点;
			v原始顶点.m坐标 = vp这->m坐标;
			v原始顶点.m出现 = vp这->m出现.m帧;
			v原始顶点.m消失 = vp这->m消失.m帧;
			v原始顶点.m颜色[0] = &vp这->m颜色[0];
			v原始顶点.m颜色[1] = &vp这->m颜色[1];
		...
		//
		C子弹顶点::C曲线 v子弹顶点{this, (int)va原始顶点.size()};
		v子弹顶点.m标志[C子弹顶点::e环] = vi环;
		v子弹顶点.m中心.m出现 = m出现.m帧;
		m图形缓冲->m绘制->f生成连续点(m图形缓冲->m图形数据, v子弹顶点, va原始顶点);
	} else {	//单个子弹
		C子弹顶点::C点 v子弹顶点{this};
		v子弹顶点.m中心.m出现 = m出现.m帧;
		v子弹顶点.m中心.m消失 = 0;
		m图形缓冲->m绘制->f生成点(m图形缓冲->m图形数据, v子弹顶点);
	}
}
...
void C连续子弹::f扩展_自动细分() {
	...
	const auto f细分 = [v判定直径乘二 = f扩展_g细分间隔()](C连续子弹 &a前, C连续子弹 &a后) {
...
void C连续子弹::f连续子弹_连接(C连续子弹 &a子弹0, C连续子弹 &a子弹1) {
	//连续子弹连接锁;
	assert(!a子弹0.m后);	//必需是空指针
	assert(!a子弹1.m前);
	f连续子弹_连接1(a子弹0, a子弹1);
}
void C连续子弹::f连续子弹_断开(C连续子弹 &a子弹0, C连续子弹 &a子弹1) {
	//连续子弹连接锁;
	assert(a子弹0.m后);
	assert(a子弹1.m前);
	f连续子弹_断开1(a子弹0, a子弹1);
}
void C连续子弹::f连续子弹_细分(C连续子弹 &a前, C连续子弹 &a后, int a数量) {
	assert(a数量 > 0);
	C子弹制造机 v子弹制造机 = C游戏::fg内容().f工厂_子弹();
	v子弹制造机.m参数.fs子弹基础(a前);
	C连接工具 v连续{f连续子弹_连接1};
	v连续 += a前.shared_from_this();
	...
	v连续 += a后.shared_from_this();
}
void C连续子弹::f连续子弹_连接1(C连续子弹 &a子弹0, C连续子弹 &a子弹1) {
	a子弹0.m后 = a子弹1.shared_from_this();
	a子弹1.m前 = a子弹0.shared_from_this();
}
