...
//	}
//}
//void C画子弹_线条::f绘制网格(const C子弹顶点::C网格 &a网络信息, const C子弹顶点::C网格::ta顶点 &a数组) {
//	const C子弹 *const v子弹 = a网络信息.m子弹;
//	const S子弹属性 *v子弹属性 = v子弹->m子弹属性;
//	const t向量2 v显示 = v子弹属性->fg显示(v子弹->m缩放);
//	const float v宽数 = (float)a网络信息.m宽;
//	const float v高数 = (float)a网络信息.m高;
//	const float v半宽 = v显示.x / (float)a网络信息.m宽;
//	const float v半高 = v显示.y / (float)a网络信息.m高;
...
//	t颜色 v颜色 = a网络信息.m子弹->m颜色[0];
//	m画图->fs颜色(v颜色);
//	m画图->f绘制旋转矩形(t旋转矩形(m坐标, v显示, v方向));
//	//画圆
//	数学::C二维序号计算<int> f序号{a网络信息.m宽};
//	for (int y = 0; y != a网络信息.m高; ++y) {
//		const float v位置y = t位置计算::f右到中(y, v高数 - 1) * v全高;
//		for (int x = 0; x != a网络信息.m宽; ++x) {
//			const float v位置x = t位置计算::f右到中(x, v宽数 - 1) * v全宽;
//			v颜色.a = f计算透明度(a网络信息.m中心.m出现, a数组[f序号(y, x)].m消失);
//			m画图->fs颜色(v颜色);
