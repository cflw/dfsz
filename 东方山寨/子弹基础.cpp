...
void C子弹::f动作_旋转d(float a度, float a时间) {
	const float v过秒 = m游戏速度->fg秒();
	m速度.fs旋转d(a度 * v过秒 / a时间);
}
void C子弹::f动作_旋转r(float a弧度, float a时间) {
	const float v过秒 = m游戏速度->fg秒();
	m速度.fs旋转r(a弧度 * v过秒 / a时间);
}
...
void C子弹::f动作_加速度(float a加数, float a时间) {
	const float v过秒 = m游戏速度->fg秒();
	m速度 = m速度.fg归一() * (m速度.fg大小() + a加数 * v过秒 / a时间);
}
...
namespace cflw::数学 {
template<> 东方山寨::S子弹出现 f插值<东方山寨::S子弹出现>(const 东方山寨::S子弹出现 &a前, const 东方山寨::S子弹出现 &a后, float a插值) {
	东方山寨::S子弹出现 v出现;
	v出现.m帧 = 数学::f插值<float>(a前.m帧, a后.m帧, a插值);
	return v出现;
}
template<> 东方山寨::S子弹消失 f插值<东方山寨::S子弹消失>(const 东方山寨::S子弹消失 &a前, const 东方山寨::S子弹消失 &a后, float a插值) {
	东方山寨::S子弹消失 v消失;
	v消失.m帧 = 数学::f插值<float>(a前.m帧, a后.m帧, a插值);
	v消失.m目标 = 数学::f插值<float>(a前.m目标, a后.m目标, a插值);
	v消失.m速度 = 数学::f插值<float>(a前.m速度, a后.m速度, a插值);
	return v消失;
}
}	//namespace cflw::数学