...
public:
	void f接口_初始化() override {
		m模型_夜空 = 脚本::fg模型(L"模型.夜空.低空");
		m纹理_夜空 = 脚本::fg纹理(L"纹理.夜空.低空");
		m相机.m坐标 = {0, 0, 100};
		m相机.m目标 = {0, 100, 0};
		m相机.m上方 = {0, 0, 1};
	}
...
public:
		...
		void f事件_执行() override {
			if (m计时.f滴答()) {
				...
				for (auto v循环0 : v子弹参数.f循环(600)) {
					...
					v子弹工厂.f产生子弹<C子弹0>(v子弹参数, this, v圈数 * 数学::c二π<float>, m旋转角 + vf随机数());
				}
				m旋转角 = -m旋转角;
			}
		}
	...
	class C弹幕_测试难度 : public I关卡事件 {
		...
		void f事件_执行() override {
			if (m计时.f滴答()) {
				...
				v子弹参数.m颜色[0] = t颜色::c红;
				v子弹参数.m坐标 = {0, 100};
				v子弹参数.m速度 = t向量2::fc方向r(m速度, 0);
				for (auto v循环0 : v子弹参数.f循环(m数量)) {
					v循环0.f变换_圆形();
	...
	public:
		...
		void f事件_初始化() override {
			const auto &v遮罩制造机 = C游戏::fg内容().f工厂_遮罩();
			v遮罩制造机.f产生遮罩<C遮罩0>(0);
		}
		void f事件_执行() override {
			if (!m计时.f滴答()) {
			...
			t子弹参数 v子弹参数;
			v子弹参数.m颜色[0] = t颜色::c红;
			v子弹参数.m样式 = (int)E子弹::e鳞弹;
			v子弹参数.m坐标 = {0, 100};
			v子弹参数.m速度 = t向量2::fc方向r(100, m方向);
			v子弹工厂.f产生子弹<C子弹0>(v子弹参数);
			m方向 += 0.1f;
		}
		float m方向 = 0;
		C计时器 m计时{0.01f};
	...
	public:
		...
		void f事件_执行() override {
			...
			if (m计时.f滴答()) {
				const auto &v子弹工厂 = C游戏::fg内容().f工厂_子弹();
				t子弹参数 v子弹参数;
				v子弹参数.m坐标.x = m边框.m矩形.fg中心().x;
				v子弹参数.m坐标.y = 100;
				v子弹参数.m速度 = t向量2::fc方向r(200, m时间 * 10);
				v子弹工厂.f产生子弹<C子弹0>(v子弹参数, m边框);
			}
		}
	...
	public:
		...
		public:
			...
			void f事件_执行() override {
				...
				if (m计时_发射.f滴答()) {
					...
					t子弹参数 v子弹参数;
					v子弹参数.m坐标 = m坐标;
					v子弹参数.m速度 = t向量2::fc方向r(50, m方向);
					for (auto v循环0 : v子弹参数.f循环(10)) {
						v循环0.f变换_圆形();
						v子弹工厂.f产生子弹<C普通子弹>(v子弹参数);
					}
					m方向 += 1;
			...
			void f事件_击破() override {
				...
				图形模板::f敌机死亡小爆炸(*this);
				//道具
				const auto &v道具工厂 = C游戏::fg内容().f工厂_道具();
				t道具参数 v道具参数;
				v道具参数.m坐标 = m坐标;
				v道具参数.m道具数量 = 10;
				v道具参数.fs属性((int)E道具::e火力);
				v道具工厂.f产生道具(v道具参数);
			}
		private:
			float m方向 = 0;
		...
		void f事件_执行() override {
			...
			v敌机参数.m生命值 = 1000;
			auto v敌机 = v敌机工厂.f产生敌机<C敌机0>(v敌机参数);
			图形模板::f分血条(*v敌机);
	...
	public:
		...
		void f事件_初始化() {
			m道具参数.m坐标 = t向量2(0, 50);
			const auto &v遮罩工厂 = C游戏::fg内容().f工厂_遮罩();
			v遮罩工厂.f产生遮罩<C遮罩0>(t圆形::fc坐标半径(t向量2(0, -50), 50));
		}
		void f事件_执行() override {
			const auto &v道具工厂 = C游戏::fg内容().f工厂_道具();
			if (m计时.f滴答()) {
				v道具工厂.f产生道具(m道具参数, m道具组);
			}
			v道具工厂.f产生道具(m道具参数, m道具组);
			f动作_结束();
		}
	...
	class C王战0_1 : public I王战事件 {
		...
		void f事件_初始化() override {
			...
			v王->fs生命值(1000);
			m王战->fs弹幕时间(5);
			m王战->f发动符卡(L"符卡", 10000);
			f计算难度();
			m背景 = 图形模板::f符卡淡入背景(*脚本::fg纹理(L"纹理.符卡背景.夜雀0"));
	...
	void f事件_初始化() override {
		...
		//v脚本.f事件<C王战0>();
		//v脚本.f等待(1);
		//v脚本.f事件(关卡效果::F显示标题({L"主标题", L"场景"}));

		//对话
	
		C对话脚本 v对话;
		auto v角色0 = v对话.f创建角色(0, E对话方向::e从左向右);
		auto v角色1 = v对话.f创建角色(1, E对话方向::e从右向左);
		//static C空白立绘建造机 v立绘;
		std::shared_ptr<C静态立绘建造机> v立绘 = std::make_shared<C静态立绘建造机>(*C游戏::fg资源().fg静态立绘().ma属性[L"立绘.纳兹琳"]);
		const auto &va文本 = C文本管理::fg实例().fg普通文本();
		v角色0.f显示立绘(v立绘);
		v角色0.f对话(va文本[L"s1.boss0.name"]);
		v角色1.f显示立绘(v立绘);
		v角色1.f对话(va文本[L"s1.boss1.name"]);
		v角色0.f立绘聚焦();
		v角色0.f立绘表情(E立绘表情::e高兴);
		v角色0.f显示标题({L"名称0", L"称号0"});
		v角色0.f对话(va文本[L"s1a.0"]);
		v角色1.f立绘聚焦();
		v角色1.f立绘表情(E立绘表情::e伤心);
		v角色1.f显示标题({L"名称1", L"称号1"});
		v角色1.f对话(va文本[L"s1a.1"]);
		v脚本.f对话(v对话);
...
public:
	...
	void f事件_初始化() override {
		...
		m控制->f设置场景(nullptr);
		m控制->f动作_结束关卡(1);
		v脚本.f事件(关卡效果::F显示标题({L"测试关卡1", L"场景1"}));
	}
};
...
public:
	...
	public:
		void f事件_执行() override {
			...
			v子弹参数.fs长度(200);
			v子弹参数.fs宽度(200);
			v子弹参数.fs样式((int)E子弹::e大光弹);
			v子弹参数.fs绘制((int)E图形管线::e子弹正常);
			for (auto v循环0 : v子弹参数.f循环(1)) {
				v循环0.f变换_圆形();
				v子弹工厂.f产生子弹<C大子弹>(v子弹参数);
			}
			//v子弹工厂.f产生子弹<C大子弹>(v子弹参数);
			f动作_结束();
		}
	...
	public:
		...
		void f事件_执行() override {
			if (m计时.f滴答()) {
				...
				t子弹参数 v子弹参数;
				v子弹参数.m速度 = {200, 0};
				v子弹参数.fs样式((int)E子弹::e曲线激光);
				v子弹参数.fs长度(200);
				v子弹参数.fs绘制((int)E图形管线::e子弹正常);
				for (auto v循环0 : v子弹参数.f循环(36)) {
					v循环0.f变换_圆形();
					v子弹工厂.f产生子弹<C激光0>(v子弹参数, m旋转);
				}
				m旋转 = -m旋转;
			}
		}
	...
	public:
		...
		void f事件_执行() override {
			if (m计时.f滴答()) {
				...
				t子弹参数 v子弹参数;
				v子弹参数.m速度 = {0, -100};
				v子弹参数.fs样式((int)E子弹::e环圆弹);
				v子弹参数.fs绘制((int)E图形管线::e子弹正常);
				auto v连续 = C连续子弹::C环形工具();
				for (auto v循环0 : v子弹参数.f循环(c数量)) {
					v循环0.f变换_圆形();
					auto vp子弹 = v子弹工厂.f产生子弹<C连续子弹>(v子弹参数);
					v连续 += vp子弹;
				}
			}
		}
	...
	public:
		void f事件_执行() override {
			...
			t子弹参数 v子弹参数;
			v子弹参数.m速度 = {50, 0};
			v子弹参数.fs样式((int)E子弹::e米弹);
			v子弹参数.fs绘制((int)E图形管线::e子弹正常);
			v子弹参数.fs长度(20);
			for (auto v循环0 : v子弹参数.f循环(32)) {
				v循环0.f变换_圆形();
				v子弹工厂.f产生子弹<C直线激光>(v子弹参数);
			}
			f动作_结束();
		}
	};
