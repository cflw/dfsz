...
void C关卡标题::f兼容显示() const {
	...
	v画文本.fs颜色(t颜色::c白);
	const float v文本透明度 = 数学::f等腰梯形插值<float>(0, 1, 1.f / m寿命, fg生命百分比());
	v画文本.fs透明度(v文本透明度 * C抬显控制::fg全局透明度());
...
C总血条::C总血条(int a):
	...
	m渲染目标 = v二维->fg渲染目标();
	m画笔 = v二维->fc纯色画笔(t颜色(0, 1, 0, c血条透明度));
}
void C总血条::f接口_更新() {
	auto &v图形 = C游戏::fg图形();
	...
	m画笔->SetOpacity(c血条透明度 * C抬显控制::fg全局透明度());
}
