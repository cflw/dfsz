...
// 顶点工厂
//==============================================================================
C顶点矩形工厂::C循环::C循环(C顶点矩形工厂 &a工厂, int a列, int a行):
	m参数(&a工厂.m参数),
	m行(a行), m列(a列), i(0), j(0),
	m参数备份(a工厂.m参数) {
}
...
void C顶点矩形工厂::f创建矩形(int a标识) {
	assert(ma顶点矩形->fi不存在(a标识));
	S顶点矩形 v矩形 = fc顶点矩形(m参数);
	ma顶点矩形->f添加(a标识, v矩形);
	m参数.m标志.reset();
}
void C顶点矩形工厂::f复制矩形(int a新, int a旧) {
	ma顶点矩形->f引用(a新, a旧);
}
C顶点矩形工厂::C循环 C顶点矩形工厂::f循环(int a列, int a行) {
	return C循环(*this, a列, a行);
}
void C顶点矩形工厂::f自动化_完整纹理(int a标识, const t向量2 &a纹理尺寸) {
	m参数.fs裁剪尺寸(a纹理尺寸);
	m参数.fs纹理尺寸(a纹理尺寸);
	m参数.fs裁剪坐标(t向量2::c零);
	f创建矩形(a标识);
}
void C顶点矩形工厂::f自动化_完整切割(int a标识, const t向量2 &a纹理尺寸, const t向量2 &a裁剪尺寸) {
...
void C顶点矩形工厂::f自动化_完整切割(int a标识, const t向量2 &a纹理尺寸, int a列, int a行) {
